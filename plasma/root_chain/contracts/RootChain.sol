...
contract RootChain {
        ...
        public payable onlyWithValue(EXIT_BOND)
    {
        ...
        addExitToQueue(_depositPos, msg.sender, _token, _amount, 'test', plasmaBlocks[blknum].timestamp);
    }
    ...
    function startFeeExit(address _token, uint256 _amount) public payable onlyOperator onlyWithValue(EXIT_BOND) {
        addExitToQueue(currentFeeExit, msg.sender, _token, _amount, 'test', block.timestamp + 1);
        ...
        public payable onlyWithValue(EXIT_BOND)
    {
        ...
        // for testing
        //var txList = RLPDecode.toList(RLPDecode.toRlpItem(_txBytes));
        //bytes memory encodedMapping = RLPDecode.toBytes(txList[12]);

        //emit forDebug01(blknum, txindex, oindex, encodedMapping);

        addExitToQueue(_utxoPos, exitingTx.exitor, exitingTx.token, exitingTx.amount, exitingTx.dmapping, plasmaBlocks[blknum].timestamp);
    }
    ...
    function finalizeExits(address _token) public {
        ...
        emit FinalState(currentExit.owner, currentExit.amount, currentExit.owner.balance,
          currentExit.dmapping);
    }
